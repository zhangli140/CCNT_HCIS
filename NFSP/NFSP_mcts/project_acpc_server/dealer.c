...
#define DEFAULT_MAX_RESPONSE_MICROS 60000000000
#define DEFAULT_MAX_USED_HAND_MICROS 60000000000
#define DEFAULT_MAX_USED_PER_HAND_MICROS 700000000
...
static int readPlayerResponse(const Game *game,
	...
	const int seatFD)                            // 10.29 const int seatFD
{
	int c, r;
	...
	while (1)
	{
		...
		if (line[0] == 'Q')
		{
			...
			if (c < 0)
			{
				...
				fprintf(stderr, "WARNING: bad state format in response123\n");
				continue;
			}
			...
			/*if (!matchStatesEqual(game, state, &tempState))
			{
				...
				fprintf(stderr, "WARNING: ignoring un-requested response123\n");
				continue;
			}*/
...
doneRead_1:
			...
			if (write(seatFD, new_line, c) != c)
			{
				...
				fprintf(stderr, "123ERROR: could not send query to seat\n");
				return -1;
			}
	...
	FILE *logFile, FILE *transactionFile)
{
	...
	while (1)
	{
		...
		/* play the hand */                                 // \BD\F8\D0\D0һ\BE\D6\CD\EA\D5\FB\D3\CEϷ
		while (!stateFinished(&state.state))
		{
			...
			if (readPlayerResponse(game, &state, quiet, currentSeat, &sendTime,
				...
				&action, &recvTime, seatFD[currentSeat]) < 0)                       // 10.29  seatFD[currentSeat]
			{
				/* error messages already handled in function */
